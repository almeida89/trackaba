...
export default function PaginaRelatorios() {
  ...
  const [criancaId, setCriancaId] = useState<string>(
    CRIANCAS_PARA_RELATORIO[0].id
  );
  const [dataInicio, setDataInicio] = useState(trintaDiasAtras());
  const [dataFim, setDataFim] = useState(hojeISO());
  const [responsavel, setResponsavel] = useState("Dra. Ana Souza");
  ...
  const historicoFiltrado = useMemo(() => {
    ...
    return historico
      .filter((h) => {
        const okBusca =
          !t ||
          h.criancaNome.toLowerCase().includes(t) ||
          h.geradoPor.toLowerCase().includes(t);
  ...
  const gerar = () => {
    ...
    const crianca = CRIANCAS_PARA_RELATORIO.find((c) => c.id === criancaId);
    if (!crianca) return;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Gere relatórios clínicos em PDF e consulte o histórico de emissões
        </p>
      </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div
                        className={`p-2 rounded-lg ${t.cor}`}
                      >
